...
- Hosted Frontend CI and Database-from-zero CI: **PASS** on the final `d1b2576` commit. Staging now has the three audited forward migrations; the final ASCII-only runtime migration was verified remotely for the Saved Views table/RPCs/RLS, `chr(243)` construction, no mojibake and appointment status assignment. Staging deployment is READY at `https://crm-odontologia-staging.vercel.app`. Authenticated Chrome QA passed on owner session for Trabajo, Pacientes, Oportunidades, the Saved Views dialog/Escape behavior and the final review routes. Block A is ready for manual sign-off; no reception-session mutation was performed.
- Blocks B–K: not started; no claim of completion.

### Regression discovered by broader SQL verification

`operational-integrity.sql` had an outdated reintake fixture (different treatment). Corrected it to test same-treatment repair and assert the exact original opportunity ID. This exposed **real baseline defects**, confirmed in the live Staging function definition: `20260916180630` used mojibake outcome labels (`AsistiÃ³`), rejecting the UI's canonical UTF-8 attendance and no-show; it also preserved opportunity status on the appointment itself during cancellation. Forward-only `20260924001122_fix_appointment_outcome_encoding.sql` preserves the existing atomic SECURITY DEFINER contract and appointment-state correction, but its remote application demonstrated that non-ASCII SQL literals are re-encoded by the migration transport. Third forward-only `20260924003500_fix_appointment_outcome_unicode_runtime.sql` constructs canonical labels with `chr(243)` inside PostgreSQL; final hosted CI and Staging verification passed (`uses_chr_243`, `no_mojibake`, `appointment_status_follows_outcome`). No historical rows or old migrations were edited. Operational integrity, workflow E2E and scoring suites pass in the isolated PGlite/database-from-zero verification. Cancellation has a dedicated retry assertion for appointment state, preserved commercial state and no duplicated work/events.
