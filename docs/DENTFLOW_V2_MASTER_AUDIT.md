...
| DF-004 | MEDIUM | Work | Pending/Followups/Tasks overlap | routing and nextActions | /trabajo compatibility entry only; no Work Center | VERIFIED STATIC |
...
| DF-007 | MEDIUM | CI | Existing npm ci, tests, Playwright, build, diff check | .github/workflows/ci.yml | Existing npm script now includes query + DF-009 contracts | VERIFIED STATIC workflow; local execution recorded; hosted CI NOT RUN |
| DF-008 | MEDIUM | Migrations | No automated from-zero / base-release upgrade job | CI | New rollback SQL regressions executed against staging | PASS targeted upgrade; full fresh database NOT RUN |
