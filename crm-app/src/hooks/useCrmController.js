...
export default function useCrmController({ session, authError }) {
  ...
  const navigateView = (view) => navigate(viewPath(view));
  ...
  const selectedLead = useMemo(() => leads.find((lead) => lead.id === selectedLeadId && lead.contact_id === selectedContactId) || null, [leads, selectedLeadId, selectedContactId]);
  ...
  function handleLeadSelect(leadId) {
    ...
    if (lead?.contact_id) navigate(opportunityPath(lead.contact_id, lead.id));
  }
  ...
  async function createManualLead(form, { scheduleAfterSave = false } = {}) {
    ...
    try {
      ...
      if (scheduleAfterSave) {
        ...
        navigate(opportunityPath(createdLead.contact_id, createdLead.id));
      }
  ...
  async function handleLogout() {
    ...
    if (logoutError) { setError(humanizeCrmError(logoutError)); return; }
