...
begin
  ...
  if normalized_outcome not in ('Confirmado', 'AsistiÃ³', 'No AsistiÃ³', 'Cancelado') then
    raise exception using errcode = '22023', message = 'Resultado de cita invÃ¡lido';
  end if;
  ...
  if not app_private.has_role(appointment_record.clinic_id, array['admin', 'owner', 'receptionist']) then
    raise exception using errcode = '42501', message = 'No tenÃ©s acceso a esta cita';
  end if;
  ...
  if normalized_outcome = 'Confirmado' then
    ...
    if appointment_at <= now() then
      raise exception using errcode = '22023', message = 'No se puede confirmar una cita cuya hora ya pasÃ³';
    end if;
  elsif normalized_outcome in ('AsistiÃ³', 'No AsistiÃ³') then
  ...
  case normalized_outcome
    when 'Confirmado' then
      ...
      task_description := 'Al finalizar la cita, registrar si el paciente asistiÃ³.';
    ...
    when 'No AsistiÃ³' then
      next_action_value := 'Recuperar paciente que no asistiÃ³';
      next_followup_value := now();
      task_title := 'Recuperar paciente que no asistiÃ³';
      task_description := 'Contactar para conocer quÃ© ocurriÃ³ y ofrecer una nueva fecha.';
  ...
  set status = case when normalized_outcome = 'Cancelado' then lead_record.status else normalized_outcome end,
      ...
      attended_at = case when normalized_outcome = 'AsistiÃ³' then now() else attended_at end,
      no_show_at = case when normalized_outcome = 'No AsistiÃ³' then now() else no_show_at end,
