-- Forward-only runtime repair. The migration transport previously re-encoded
-- non-ASCII literals; construct canonical Spanish outcome labels inside Postgres.
...
declare
  ...
  attended_outcome text := 'Asisti' || chr(243);
  no_show_outcome text := 'No Asisti' || chr(243);
...
begin
  ...
  if normalized_outcome is null
     or normalized_outcome not in ('Confirmado', attended_outcome, no_show_outcome, 'Cancelado') then
    raise exception using errcode = '22023', message = 'Resultado de cita invalido';
  end if;
  ...
  if not app_private.has_role(appointment_record.clinic_id, array['admin', 'owner', 'receptionist']) then
    raise exception using errcode = '42501', message = 'No tenes acceso a esta cita';
  end if;
  ...
  if normalized_outcome = 'Confirmado' then
    ...
    if appointment_at <= now() then
      raise exception using errcode = '22023', message = 'No se puede confirmar una cita cuya hora ya paso';
    end if;
  elsif normalized_outcome in (attended_outcome, no_show_outcome) then
    ...
    raise exception using errcode = '22023', message = 'La cita ya no puede cancelarse desde el estado actual';
  end if;
  ...
  case normalized_outcome
    when 'Confirmado' then
      ...
      task_description := 'Al finalizar la cita, registrar si el paciente asistio.';
      task_type := 'attendance';
      event_type_value := 'appointment_confirmed';
    when attended_outcome then
    ...
    when no_show_outcome then
      next_action_value := 'Recuperar paciente que no asistio';
      next_followup_value := now();
      task_title := 'Recuperar paciente que no asistio';
      task_description := 'Contactar para conocer que ocurrio y ofrecer una nueva fecha.';
  ...
  set status = normalized_outcome,
      ...
      attended_at = case when normalized_outcome = attended_outcome then now() else attended_at end,
      no_show_at = case when normalized_outcome = no_show_outcome then now() else no_show_at end,
